...
char *ReadBuffGPWInfo(FILE *fp, int *iNbuf, int *iComplexFlag, int *iNLatBuf, int *iLatNbSzBuf,
  ...
  *iNGPWDistWeights = 0;
  ...
  while (read) {
    ...
    else if (CheckWords(ctmp2, "GPWShiftFlag") == 0) {
      *iGPWShiftFlag = readVal;
      IgnoreAddLineGPWDef++;
    }
...
int ReadDefFileNInt(char *xNameListFile, MPI_Comm comm) {
  ...
  NPara = NProj + NGPWIdx + NGPWTrnLat + NGPWDistWeights + NRBMTotal + NSlater + NOptTrans + NProjBF;
  ...
  NTotalDefInt = Nsite /* LocSpn */
                 ...
                 + GPWDistWeightIdxSz /* GPWDistWeightIdx */
  ...
  if (rank == 0) {
    for (iKWidx = KWLocSpin; iKWidx < KWIdxInt_end; iKWidx++) {
      ...
      switch (iKWidx) {
        ...
        case KWGPW:
          fgets(ctmp, sizeof(ctmp) / sizeof(char), fp);
          for (i = 0; i < IgnoreAddLineGPWDef; i++) {
            fgets(ctmp, sizeof(ctmp) / sizeof(char), fp);
          }
    ...
    for (iKWidx = KWInGutzwiller; iKWidx < KWIdxInt_end; iKWidx++) {
      ...
      switch (iKWidx) {
        ...
        case KWInGPW:
          ...
          for (i = 0; i < NGPWIdx; i++) {
            fscanf(fp, "%d %lf %lf \n", &idx, &tmp_real, &tmp_comp);
            GPWVar[idx] = tmp_real + I * tmp_comp;
          }
          break;
        ...
        case KWInGPWDistWeights:
          ...
          for (i = 0; i < NGPWDistWeights; i++) {
            fscanf(fp, "%d %lf %lf \n", &idx, &tmp_real, &tmp_comp);
            GPWDistWeights[idx] = tmp_real + I * tmp_comp;
          }
          break;
...
int GetInfoGPW(FILE *fp, int *trnSize, int *trnNeighbours, int *trnLattices, int *trnCfg,
  ...
  int *optDistWeights = optTheta + 2 * NGPWTrnLat;
