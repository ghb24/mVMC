...
void VMCMainCal(MPI_Comm comm, MPI_Comm commSampler) {
  ...
  int i,info,tmp_i,j,offset,idx,f,targetBasis,k,l;

  /* optimazation for Kei */
  const int nProj=NProj;
  const int nGPWIdx=NGPWIdx;
  const int nGPWTrnLat=NGPWTrnLat;
  const int nGPWDistWeights=NGPWDistWeights;
...
#endif
      e = CalculateHamiltonian_real(creal(ip),eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum);
    }else{
#ifdef _DEBUG_VMCCAL
      printf("  Debug: sample=%d: calculateHam_cmp \n",sample);
#endif
      e = CalculateHamiltonian(ip,eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum);
    }
    //printf("MDEBUG: %lf %lf \n",creal(e),cimag(e));
    StopTimer(41);
...
#pragma loop noalias
      ...
      if (nGPWDistWeights > 0) {
        ...
        //TODO: Clean this up, fast update?
        for (i = 0; i < nGPWIdx; i++) {
          int matOffset, latId, trnSize, plaqSize, id;
          ...
          for (j = 0; j < shiftTrn; j+=translationSys) {
            for (k = 0; k < plaqSize; k++) {
              sumTargetLat = 0.0;
              #pragma omp parallel for default(shared) private(l, id) reduction(+:sumTargetLat)
              for (l = 0; l < shiftSys; l+=translationSys) {
                id = GPWSysPlaquetteIdx[latId][l*plaqSize+k]*trnSize +
                     GPWTrnPlaquetteIdx[latId][j*plaqSize+k];
                if (!signbit(inSum[id])) {
              ...
              srOptO[(offset+GPWDistWeightIdx[i][trnSize*k + j])*2] -= GPWVar[i] * sumTargetLat * 2.0 * GPWDistWeights[GPWDistWeightIdx[i][trnSize*k + j]];
              srOptO[(offset+GPWDistWeightIdx[i][trnSize*k + j])*2 + 1] -= GPWVar[i] * sumTargetLat * I * 2.0 * GPWDistWeights[GPWDistWeightIdx[i][trnSize*k + j]];
            }
          }
        }
      ...
      if (UseOrbital) {
        StartTimer(42);
        /* SlaterElmDiff */
        SlaterElmDiff_fcmp(SROptO+2*offset,ip/RBMVal(eleNum),eleIdx); //TBC: using InvM not InvM_real
        StopTimer(42);
      ...
      if(FlagOptTrans>0) { // this part will be not used
        calculateOptTransDiff(SROptO+2*offset, ip/RBMVal(eleNum)); //TBC
      }
...
#pragma loop noalias
      ...
      if(RealEvolve==1) CalculateGreenFunc(w,ip,eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum);
      StopTimer(43);
...
#endif
      CalculateGreenFunc(w,ip,eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum);
      StopTimer(42);
  ...
  fprintf(stdout, "Debug: Start: Lanczos\n");
        ...
        StartTimer(43);
        if(AllComplexFlag==0) {
          LSLocalQ_real(creal(e),creal(ip),eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum, LSLQ_real);
          calculateQQQQ_real(QQQQ_real,LSLQ_real,w,NLSHam);
        }else{
          LSLocalQ(e,ip,eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum, LSLQ);
        ...
        if(NLanczosMode>1){
          ...
          if(AllComplexFlag==0) {
            ...
            LSLocalCisAjs_real(creal(e),creal(ip),eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum);
            ...
            LSLocalCisAjs(e,ip,eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum);
