...
void VMCMainCal(MPI_Comm comm) {
  ...
  int i,info,tmp_i,j,offset,idx,f,targetBasis;
...
#pragma loop noalias
      ...
      for(f = 0; f < nRBMVis; f++) {
        ...
        for(i = 0; i < Nsite2; i++) {
          srOptO[(offset+f)*2]     += (eleNum[i]*2-1)*Nsite2;    // even real
          srOptO[(offset+f)*2+1]   += (eleNum[i]*2-1)*Nsite2*I;  // odd
        }
      }
      offset += nRBMVis;
