...
//  double         *srOptO_real = SROptO_real;
  ...
  for(sample=sampleStart;sample<sampleEnd;sample++) {
...
#endif
    e  = CalculateHamiltonian_fsz(ip,eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum,eleSpn);//fsz
    Sz = CalculateSz_fsz(ip,eleIdx,eleCfg,eleNum,eleProjCnt,eleGPWKern,eleGPWInSum,eleSpn);//fsz
    //printf("MDEBUG: Sz=%lf \n",Sz);
    ...
    if(NVMCCalMode==0) {
      ...
      CalculateGreenFunc_fsz(w,ip,eleIdx,eleCfg,eleNum,eleSpn,eleProjCnt,eleGPWKern,eleGPWInSum);
      StopTimer(42);
